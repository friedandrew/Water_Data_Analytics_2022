...
class(NCdata_processed$ActivityStartDate)
```
...
```{r}
ggplot(NCdata_processed_more, aes(x = TP_mgL, y = Chla_ugL))+
...
> I anticipate that lakes in the piedmont and coastal plain will have higher TP values doe to agricultural runoff
