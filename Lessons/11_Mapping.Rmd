...
secchi.summary.min <- LAGOScombined %>%
  filter(sampledate > "2011-12-31") %>% 
  group_by(lagoslakeid) %>%
  summarise(secchi.min = min(secchi),
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
            lat = mean(nhd_lat), 
            long = mean(nhd_long)) %>%
...
secchi.spatial.min <- st_as_sf(secchi.summary.min, coords = c("long", "lat"), crs = 4326)

ggplot() +
  geom_sf(data = states.subset.min, fill = "white") +
...
```{r}
secchi.summary.maine <- LAGOScombined %>%
  filter(state == "ME") %>% 
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha),
...
maine.sf <- filter(states, ID == "maine")

maine.plot.area <- ggplot() +
  geom_sf(data = maine.sf, fill = "white") +
...
```{r}
plot_grid(maine.plot.area, maine.plot.depth)
