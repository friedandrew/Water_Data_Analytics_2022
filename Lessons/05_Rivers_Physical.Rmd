...
EnoDischarge <- EnoDischarge %>%
  mutate(Baseflow_lfstat = baseflow(EnoDischarge$Discharge), 
...
EnoDischarge_summary <- EnoDischarge %>%
 ...
 mutate_if(is.numeric, round, 2) # rounding to only two decimal places
...
ArkansasDischarge <- ArkansasDischarge %>%
  mutate(Baseflow_lfstat = baseflow(ArkansasDischarge$Discharge), 
         Stormflow_lfstat = Discharge - Baseflow_lfstat,
         Year = year(Date), 
         WaterYear = water_year(Date))
