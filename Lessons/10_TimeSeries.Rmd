...
> aggregation: Taking data to different time steps. Removes temporal autocorrelation taking from daily to monthly

> subsampling: Build a model with some data, then test with a different set of data. 

> interpolation: Using surrounding data t ofill in gaps
...
>Positive trend over time, salinity is increasing. There is an increase in conductivity in high flow events, not low flow events
...
>Positive, but smaller in magnitude. Concentrated in months 8, 9, 10, which is hurricane season
